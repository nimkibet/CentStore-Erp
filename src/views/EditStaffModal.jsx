...
export default function EditStaffModal({ isOpen, onClose, staff, onStaffUpdated, token }) {
  ...
  const KNOWN_ROLES = ['Admin', 'Manager', 'Cashier', 'HR', 'WebAdmin', 'CEO', 'Finance'];
  ...
  useEffect(() => {
    if (staff) {
      ...
      if (KNOWN_ROLES.includes(staffRole)) {
        setRole(staffRole);
        setCustomRole('');
      } else {
        setRole('custom');
        setCustomRole(staffRole);
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const finalRole = role === 'custom' ? (customRole.trim() || staff.role || 'Staff') : role;
      const payload = {
        role: finalRole,
        department,
        position,
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                {role === 'custom' && (
                  <div className="mt-2">
                    <label className="block text-xs font-medium text-slate-500 mb-1">Custom Role Title</label>
                    <input
                      type="text"
                      placeholder="e.g. InventoryLead, Auditor, Supervisor"
                      value={customRole}
